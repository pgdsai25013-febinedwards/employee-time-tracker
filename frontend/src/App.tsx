...
/**
 * App.tsx
 * Option A: Login does NOT require team selection.
 * After Google sign-in, user is created (team_id may be null).
 * Teams are fetched after login; user selects team in the Tracker UI.
 */
...
const App: React.FC = () => {
  ...
  // Teams & tasks (loaded AFTER login)
  ...
  // NEW — controls expand/collapse for each date
const [openDays, setOpenDays] = useState<Record<string, boolean>>({});
  ...
  // Exchange Google idToken -> backend
  ...
  // Initialize GSI button (only when not logged in)
  useEffect(() => {
    if (authUser) return;
  ...
  // Load tasks when selectedTeamId changes
  useEffect(() => {
  ...
  // Fetch recent logs and group by date
...
// --- FIX: Normalize dateKey to YYYY-MM-DD ---
let dateKey = "";
try {
  ...
  continue;
      ...
      const grouped = Array.from(byDate.entries()).map(([date, logs]) => ({ date, logs })).sort((a, b) => (a.date < b.date ? 1 : -1));
  ...
  const restoreActiveTimer = async () => {
    try {
      ...
      const now = Date.now();
      idleBaseRef.current = active.idle_seconds ?? 0;
      lastActivityAtRef.current = now;
      lastActivityEventAtRef.current = now;
      setIdleSeconds(idleBaseRef.current);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const handleActivity = () => {
      ...
      const now = Date.now();
      if (now - lastActivityEventAtRef.current < 250) return;
      lastActivityEventAtRef.current = now;
      const last = lastActivityAtRef.current;
      if (last != null) {
        const diff = now - last;
        if (diff > idleThresholdMs) {
          const extraIdle = Math.floor((diff - idleThresholdMs) / 1000);
          idleBaseRef.current += extraIdle;
          if (isMountedRef.current) setIdleSeconds(idleBaseRef.current);
        }
      }
      lastActivityAtRef.current = now;
    };
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      const now = Date.now();
      const diff = now - last;
      let totalIdle = idleBaseRef.current;
      if (diff > idleThresholdMs) {
        const currentStreak = Math.floor((diff - idleThresholdMs) / 1000);
        totalIdle += currentStreak;
      }
      setIdleSeconds((prev) => (prev === totalIdle ? prev : totalIdle));
    }, 1000);
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      if (!isMountedRef.current || !timerStartAtRef.current) return;
  ...
  const handleStartTimer = async () => {
    ...
    try {
      ...
      const diff = Math.floor((Date.now() - startedAtMs) / 1000);
      setElapsedSeconds(diff > 0 ? diff : 0);
      idleBaseRef.current = 0;
      lastActivityAtRef.current = Date.now();
      lastActivityEventAtRef.current = Date.now();
      setIdleSeconds(0);
...
const openEditLog = (log: any) => {
  ...
  // --- FIX: Always normalize date for <input type="date"> ---
  try {
...
const toggleDay = (date: string) => {
  ...
  // Set team for current user (if user.team_id is null)
  ...
  const isIdleNow = idleSeconds >= 60;
  const isTimerActive = currentLogId !== null;
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md bg-slate-900/80 border-slate-800">
          <CardHeader className="flex flex-col items-center gap-2 py-6">
            <img
  src="/employee-time-tracker-icon.svg"
  alt="App Logo"
  className="h-12 w-12 rounded-full"
/>

            <CardTitle className="text-lg text-center">Employee Time Tracker</CardTitle>
            <p className="text-[12px] text-slate-400 text-center max-w-xs">Sign in with your Google account to start tracking time.</p>
          </CardHeader>
          <CardContent className="flex flex-col items-center gap-4 pb-8">
            <div id="google-signin-button" className="w-full flex justify-center" />
            <div className="text-[11px] text-slate-500 text-center max-w-xs">We only use your name &amp; email to create your profile in this demo app.</div>
          </CardContent>
        </Card>
      </div>
    );
...
/>
          ...
          <div>
            ...
            <div className="text-[11px] text-slate-400 flex flex-wrap items-center gap-2">
              <span>Smart time tracking for smarter teams.</span>
              {isTimerActive && (
                <>
                  <Badge variant="secondary" className="text-[10px]">Timer #{currentLogId} · Elapsed {formatElapsed(elapsedSeconds)}</Badge>
                  <Badge variant={isIdleNow ? "outline" : "secondary"} className={`text-[10px] ${isIdleNow ? "border-red-500 text-red-300" : "bg-emerald-600/80"}`}>{isIdleNow ? "IDLE" : "ACTIVE"} · Idle {idleMinutes}m</Badge>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" className="border-slate-700 h-8 w-8" onClick={handleLogout}><LogOut className="h-4 w-4" /></Button>
        </div>
      </header>
<link rel="icon" type="image/svg+xml" href="/employee-time-tracker-icon.svg" />
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                  <div className="space-y-1">
                    ...
                    <select value={selectedTaskId} onChange={(e) => setSelectedTaskId(e.target.value === "" ? "" : Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-md px-2 py-2 text-xs sm:text-sm" disabled={!selectedTeamId || isTimerActive}>
                  ...
                  <div className="space-y-1">
                    ...
                    <Input placeholder="e.g. 120" value={volumeInput} onChange={(e) => setVolumeInput(e.target.value)} className="bg-slate-950 border-slate-700 text-xs sm:text-sm" />
                    <div className="text-[10px] text-slate-500">Enter Volume</div>
                  </div>

                  <div className="flex gap-2 justify-end">
                    <Button className="bg-emerald-500 hover:bg-emerald-600 text-xs sm:text-sm" disabled={selectedTaskId === "" || !selectedTeamId || isStarting || isTimerActive} onClick={handleStartTimer}>
                      {isStarting ? "Starting…" : "Start timer"}
                    </Button>
                    <Button variant="outline" className="border-slate-700 text-xs sm:text-sm" disabled={!isTimerActive || isStopping} onClick={handleStopTimer}>
                ...
                <div className="text-[11px] text-slate-500">
                  Idle time is auto-calculated based on keyboard/mouse inactivity (&gt;= 60s). <strong>Final productive time = Duration − Idle</strong>.
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="bg-slate-900/70 border-slate-800">
              <CardHeader className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  <Button size="icon" variant="outline" className="h-7 w-7 border-slate-700" onClick={() => { alert("Paging not implemented in demo."); }}><ChevronLeft className="h-3 w-3" /></Button>
                  <Button size="icon" variant="outline" className="h-7 w-7 border-slate-700" onClick={() => { alert("Paging not implemented in demo."); }}><ChevronRight className="h-3 w-3" /></Button>
                </div>
              </CardHeader>
              <CardContent className="space-y-2">
                {dayGroups.length === 0 && <div className="text-xs text-slate-500">No logs recorded for the last 3 days yet.</div>}
...
>
                                  ...
                                  <TableRow key={log.id}>
                                    ...
                                    <TableCell className="text-[11px] text-right">
                                      <div className="flex justify-end gap-1">
                                        <Button size="icon" variant="outline" className="h-6 w-6 border-slate-700" onClick={() => openEditLog(log)}><Edit className="h-3 w-3" /></Button>
                                        <Button size="icon" variant="outline" className="h-6 w-6 border-slate-700 text-red-400 hover:text-red-300" onClick={() => deleteLog(log)} disabled={isDeletingId === log.id}><Trash2 className="h-3 w-3" /></Button>
                                      </div>
                                    </TableCell>
                                  </TableRow>
                                );
            ...
            {editingLog && (
              <Card id="edit-panel" className="bg-slate-900/80 border-slate-800">
                <CardHeader><CardTitle className="text-sm">Edit log #{editingLog.id}</CardTitle></CardHeader>
                <CardContent className="space-y-3 text-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <select value={editTaskId} onChange={(e) => setEditTaskId(e.target.value === "" ? "" : Number(e.target.value))} className="w-full bg-slate-950 border border-slate-700 rounded-md px-2 py-2 text-xs">
                        <option value="">{selectedTeamId ? "Select task…" : "Select team at top…"}</option>
                        {tasks.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                      </select>
                      <div className="text-[10px] text-slate-500">The list is based on the currently selected team in the top-right.</div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-[11px] text-slate-400">Volume (units)</div>
                      <Input value={editVolume} onChange={(e) => setEditVolume(e.target.value)} className="bg-slate-950 border-slate-700 text-xs" />
                    </div>
                  </div>

                  <div className="flex justify-end gap-2 mt-2">
                    <Button variant="outline" className="border-slate-700 text-xs" onClick={cancelEdit} disabled={isSavingEdit}>Cancel</Button>
                    <Button className="bg-emerald-500 hover:bg-emerald-600 text-xs" onClick={saveEdit} disabled={isSavingEdit}>{isSavingEdit ? "Saving…" : "Save changes"}</Button>
                  </div>

                  <p className="text-[10px] text-slate-500 mt-1">You can edit <strong>date</strong>, <strong>task</strong>, and <strong>volume</strong>. Idle time is controlled by the system and cannot be changed here.</p>
                </CardContent>
              </Card>
            )}
          </section>
        ...
        {activeTab === "dashboard" && (
          <section className="space-y-3">
            <Card className="bg-slate-900/70 border-slate-800">
              <CardHeader><CardTitle className="text-sm">Dashboard – manager tools &amp; future metrics</CardTitle></CardHeader>
              <CardContent className="space-y-4 text-xs text-slate-300">
                <div className="space-y-1">
                  <p>Here we will show metrics like productive time, idle time, total volume, and utilization for the selected employee and period.</p>
                ...
                <div className="border border-slate-800 rounded-md p-3 bg-slate-950/60 space-y-3">
                  <div className="grid grid-cols-1 sm:grid-cols-5 gap-2 items-end">
                    <div className="space-y-1">
                      ...
                      <Input type="number" placeholder="2025" value={lockYear} onChange={(e) => setLockYear(e.target.value)} className="bg-slate-950 border-slate-700 text-xs" />
                    </div>
                    <div className="space-y-1">
                      <div className="text-[11px] text-slate-400">Month</div>
                      <select value={lockMonth} onChange={(e) => setLockMonth(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-md px-2 py-2 text-xs">
                    ...
                    <div className="flex flex-col gap-1">
                      <Button className="flex-1 bg-slate-800 hover:bg-slate-700 text-[11px]" onClick={async () => alert("Check status in demo")} disabled={!selectedTeamId}>Check status</Button>
                    </div>
                    <div className="flex flex-col gap-1">
                      <Button className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-[11px]" onClick={() => alert("Locking is demo-only")} disabled={!selectedTeamId}>Lock month</Button>
                      <Button className="flex-1 bg-slate-700 hover:bg-slate-600 text-[11px]" onClick={() => alert("Unlocking is demo-only")} disabled={!selectedTeamId}>Unlock month</Button>
                    </div>
                    <div className="space-y-1">
                      <div className="text-[11px] text-slate-400">Status</div>
                      {lockStatus === null && <div className="text-[11px] text-slate-500">No status yet. Check, lock or unlock to see result.</div>}
                      {lockStatus === "locked" && <Badge variant="secondary" className="text-[10px] bg-emerald-600/80">Locked – edits, deletes &amp; stop blocked by backend</Badge>}
                      {lockStatus === "unlocked" && <Badge variant="outline" className="text-[10px] border-amber-500 text-amber-300">Unlocked – edits &amp; deletes allowed</Badge>}
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        {activeTab === "history" && (
          <section className="space-y-3">
            <Card className="bg-slate-900/70 border-slate-800">
              <CardHeader><CardTitle className="text-sm">History Tracker – export CSV</CardTitle></CardHeader>
              <CardContent className="space-y-3 text-xs text-slate-300">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div className="space-y-1"><div className="text-[11px] text-slate-400">From date</div><DatePicker value={historyFrom} onChange={setHistoryFrom} placeholder="From date" /></div>
                  <div className="space-y-1"><div className="text-[11px] text-slate-400">To date</div><DatePicker value={historyTo} onChange={setHistoryTo} placeholder="To date" /></div>
                  <div className="flex items-end"><Button className="w-full bg-emerald-500 hover:bg-emerald-600 text-xs" onClick={handleDownloadCSV}>Download CSV</Button></div>
                </div>
                <p className="text-[11px] text-slate-500">This downloads all your time logs between these dates as a CSV file.</p>
              </CardContent>
            </Card>
          </section>
        )}
          ...
          <section className="space-y-3">
            <Card className="bg-slate-900/70 border-slate-800">
              ...
              <CardContent className="space-y-2 text-xs text-slate-300">
                <div><span className="font-semibold">Name: </span>{authUser.full_name}</div>
                <div><span className="font-semibold">Email: </span>{authUser.email}</div>
                <div><span className="font-semibold">Team: </span>{teams.find((t) => t.id === selectedTeamId)?.name ?? (authUser.team_id ? `Team #${authUser.team_id}` : "None selected")}</div>
                <div><span className="font-semibold">Role: </span>{authUser.role}</div>

                {authUser.team_id == null && (
                  <div className="mt-2">
                    <div className="text-[11px] text-slate-400 mb-1">You don't have a team assigned. Choose one below and click Save.</div>
                    <div className="flex gap-2 items-center">
                      <select value={selectedTeamId ?? ""} onChange={(e) => setSelectedTeamId(Number(e.target.value) || null)} className="bg-slate-950 border border-slate-700 rounded-md px-2 py-1 text-xs">
                        <option value="">Select team…</option>
                        {teams.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                      </select>
                      <Button className="bg-emerald-500 hover:bg-emerald-600 text-xs" onClick={() => { if (!selectedTeamId) alert("Choose a team first."); else setTeamForUser(selectedTeamId); }}>Save team</Button>
                    </div>
                  </div>
                )}

                <p className="text-[11px] text-slate-500 mt-2">Your account was created via Google sign-in. You can update your team here if needed.</p>
              </CardContent>
            </Card>
          </section>
        )}
